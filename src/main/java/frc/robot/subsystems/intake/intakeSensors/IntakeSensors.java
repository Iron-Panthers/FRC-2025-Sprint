...
public class IntakeSensors {
    ...
    public boolean isReadyToIntake() {
        if(getIsDetected() == true && getDistance() < 0.5/*FIXME: CHANGE THIS VALUE TO WHAT WE NEED*/) {
            return true;
        } else {
            return false;
        }
    } 
    //FIXME: make it so that if this method returns true, then the robot will align to or intake the piece
